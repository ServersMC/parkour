...
object CSetStart : ICommand, ITrackedEvent {
	...
	override fun getInUse(): String? = "${GRAY}${EventTracker.getSinglePlayer(this)?.name} ${RED}is already setting a start position!"
		...
		if (block.type.toString().endsWith("_PLATE", true)) {
			...
			if (CourseManager.isSensor(block.location)) {
				...
				when (sensor.getType()) {
					...
					CSensor.Type.FINISH -> player.sendMessage("${YELLOW}This is no longer the finish position!")
